...
def encryptFile(key, filename):
    ...
    IV = Random.new(16)  # Initialisierungsvektor
    encryptor = AES.new(key, AES.MODE_CFB, IV)
    with open(filename, 'rb') as f_in:  # we need to have everything in binary
        with open(out_file, 'wb') as f_out:
            f_out.write(
                file_size.encode('utf-8'))  # use utf-8 as standard in this script # write the size first in the file
            ...
            while 1:
                ...
                if len(chunk) % 16 != 0:
                    chunk += b' ' * (16 - (len(chunk) % 16))  # If the chunk is too short, we fill the empty space with binary whitespaces
